...
## Seedance Prompt 速查（视频）

视频默认走 Seedance 2.0，prompt 质量直接决定效果。**调 `video create` 前 agent 必须先按下表把用户口语 prompt 改写成完整公式**。详细公式与示例见 `references/seedance-prompt-guide.md`。
...
参考素材按上传顺序写 ``图片1`` / ``图片2`` / ``视频1``，提示词里**精确指代**。
...
| 文生视频 | `主体 + 动作 [+ 环境 + 运镜]` |
| 加广告语 | `…画面中部显示文字"<内容>"` |
| 加字幕 | `…画面底部出现字幕，字幕内容为"<台词>"，与音频同步` |
| 加气泡台词 | `<角色>说："<台词>"，周围出现气泡，气泡里写着台词` |
| 一图生视频 | `参考图片1中的<主体>，生成<画面>` |
| 多图组合 | `参考图片1中<X>，图片2中<Y>，生成<画面>` |
| 动作模仿 | `参考视频1的<动作描述>，生成<画面>，保持动作细节一致` |
| 运镜模仿 | `参考视频1的<运镜描述>，生成<画面>，保持运镜一致` |
| 特效模仿 | `参考视频1的<特效描述>，生成<画面>，保持特效一致` |
| 视频加元素 | `在视频1的<位置>，增加<元素>` |
| 视频删元素 | `删除视频1中的<元素>，其他不变` |
| 视频改元素 | `将视频1中的<原>替换为<新>，动作运镜不变` |
| 续写视频 | `生成视频1之后的内容，<新增情节>` |
| 拼接视频 | `视频1 + 过渡描述 + 接视频2`（≤3 段、总 ≤15 秒） |

### Prompt 改写规范（Agent 必读）

调 `video create` 之前，**Agent 必须**：

1. 把用户口语 prompt 套进 §三要素 至少补主体+动作。模糊的 "做个视频" 要追问或自行补全场景。
2. 涉及参考素材时改写为 `图片n` / `视频n` 显式指代，不要写 "这张图"。
3. 文字内容用常用字。生僻字、特殊符号会渲染失败。
4. 复杂任务（含字幕/气泡/视频编辑/多模态参考）拿不准时**先读 `references/seedance-prompt-guide.md`**。
5. 改写后的 prompt 直接传 `--prompt`。NL 参数提取（duration/ratio 等）仍保留，因此 prompt 里写"5秒/竖屏/不要声音"会被自动转 flag。
